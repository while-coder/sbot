...
export interface IMemoryStore {
    ...
    /** 暂存一条 remember_memory 消息，等待 turn-end Extract 统一接走。 */
    pushPendingRememberMessage(content: string, scope: MemoryScope, now: number, target: MemoryTarget): number;

    /** 入队一轮对话的消息快照；同时接走当前 target 下所有 pending remember message。 */
    ...
    /** 将目标作用域内的一条 failed job 重新放回 pending 队列。 */
    retryFailedJob(id: number, now: number, target: MemoryTarget): boolean;

    /** 删除目标作用域内的一条 failed job。返回 false 表示不存在或状态不是 failed。 */
