...
type ThreadRow = { type: string; data: string; created_at?: number; think_id?: string };
...
export class AgentFileSaver implements IAgentSaverService {
    ...
    async pushMessage(message: BaseMessage): Promise<void> {
        ...
        file.messages.push({ type, data, created_at: Math.floor(Date.now() / 1000), think_id });
        ...
        if (file.messages.length > 1000) {
            ...
            for (const row of removed) {
                if (row.think_id) delete thinks[row.think_id];
            }
        }
