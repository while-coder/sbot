...
async function installByUrl() {
  ...
  try {
    const res = await apiFetch(`/api/agents/${encodeURIComponent(agentName.value)}/skill-hub/install`, 'POST', { url, overwrite: hubUrlOverwrite.value })
...
async function confirmInstall() {
  ...
  try {
    const res = await apiFetch(`/api/agents/${encodeURIComponent(agentName.value)}/skill-hub/install`, 'POST', { url: selected.value.sourceUrl, overwrite: overwrite.value })
  ...
  <template v-if="visible">
    ...
    <div v-if="showHub" class="modal-overlay" @click.self="showHub = false">
      <div class="modal-box wide" style="max-width:960px;width:90vw;display:flex;flex-direction:column;max-height:80vh">
        <div class="modal-header" style="flex-shrink:0"><h3>{{ t('skills.install_title') }}</h3><button class="modal-close" @click="showHub = false">&times;</button></div>
        <div style="display:flex;border-bottom:1px solid #e2e8f0;flex-shrink:0;padding:0 20px">
          <button v-for="tab in ([{key:'search',label:t('skills.search_tab')},{key:'url',label:t('skills.url_install_tab')}] as const)" :key="tab.key"
        ...
        <div class="modal-body" style="flex:1;overflow:hidden;display:flex;flex-direction:column">
          <template v-if="hubTab === 'search'">
            <div style="display:flex;gap:8px;margin-bottom:16px;flex-shrink:0">
              <input v-model="hubQuery" :placeholder="t('skills.search_placeholder_hub')" style="flex:1" @keydown="hubKeydown" />
              <button class="btn-primary" :disabled="hubSearching || !hubQuery.trim()" @click="hubSearch">{{ hubSearching ? t('skills.searching') : t('skills.search_tab') }}</button>
            </div>
            <div style="flex:1;overflow-y:auto;min-height:0">
              ...
              <template v-else-if="hubSearched">
                ...
                <table v-else style="width:100%;table-layout:fixed">
                  ...
                  <thead><tr><th>{{ t('common.name') }}</th><th>{{ t('common.description') }}</th><th>{{ t('skills.version_col') }}</th><th>{{ t('skills.source_col') }}</th><th>{{ t('common.ops') }}</th></tr></thead>
                  <tbody>
          ...
          <template v-else-if="hubTab === 'url'">
            <div style="display:flex;gap:8px;margin-bottom:12px;align-items:center">
              <input v-model="hubUrlInput" :placeholder="t('skills.url_placeholder')" style="flex:1" @keydown.enter="installByUrl" />
              <button class="btn-primary" :disabled="hubUrlInstalling || !hubUrlInput.trim()" @click="installByUrl">{{ hubUrlInstalling ? '安装中...' : '安装' }}</button>
            </div>
            <label style="display:flex;align-items:center;gap:6px;font-size:13px;cursor:pointer"><input type="checkbox" v-model="hubUrlOverwrite" /> {{ t('skills.override') }}</label>
    ...
    <div v-if="showInstall && selected" class="modal-overlay" @click.self="showInstall = false">
      <div class="modal-box">
        <div class="modal-header"><h3>{{ t('skills.install_title') }}</h3><button class="modal-close" @click="showInstall = false">&times;</button></div>
        <div class="modal-body">
          ...
          <div style="padding:10px 12px;background:#f1f5f9;border-radius:6px;font-size:13px;color:#475569;margin-bottom:12px">
            {{ t('skills.install_to') }}<code style="font-family:monospace;background:#e2e8f0;padding:2px 6px;border-radius:3px">~/.sbot/agents/{{ agentName }}/skills/</code>
          </div>
          <label style="display:flex;align-items:center;gap:8px;font-size:13px;cursor:pointer"><input type="checkbox" v-model="overwrite" /> {{ t('skills.override') }}</label>
        </div>
        <div class="modal-footer">
          <button class="btn-outline" @click="showInstall = false">{{ t('common.cancel') }}</button>
          <button class="btn-primary" :disabled="installing" @click="confirmInstall">{{ installing ? '安装中...' : t('skills.confirm_install') }}</button>
        </div>
      </div>
    </div>
  </template>
