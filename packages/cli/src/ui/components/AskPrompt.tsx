  ...
  const handleKeypress = useCallback(
    (key: Key) => {
      ...
      if (question.type === 'radio') {
        ...
        if (key.name === 'up') {
          setSelectedOption((i) => (i > 0 ? i - 1 : opts.length - 1));
        } else if (key.name === 'down') {
          setSelectedOption((i) => (i < opts.length - 1 ? i + 1 : 0));
    ...
    [question, selectedOption, checkedOptions, advanceOrSubmit],
  );
    ...
    <Box flexDirection="column" borderStyle="single" borderColor={theme.status.info} paddingX={1}>
      ...
      <Box marginTop={1}>
        <Text color={theme.text.muted}>
          {question.type === 'radio' && '↑↓ select  Enter confirm'}
          {question.type === 'checkbox' && '↑↓ move  Space toggle  Enter confirm'}
...
const RadioQuestion: React.FC<{ options: string[]; selectedIndex: number }> = ({
  options,
  selectedIndex,
}) => (
  <Box flexDirection="column" marginTop={1}>
    {options.map((opt, i) => (
      <Text key={i} color={i === selectedIndex ? theme.text.accent : theme.text.primary}>
        {i === selectedIndex ? '◉ ' : '○ '}{opt}
      </Text>
    ))}
  </Box>
...
}> = ({ options, selectedIndex, checkedIndices }) => (
  <Box flexDirection="column" marginTop={1}>
    {options.map((opt, i) => {
