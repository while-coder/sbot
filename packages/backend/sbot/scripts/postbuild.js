...
// 读取 ReleaseNote.md / ReleaseNote.zh.md（位于 packages/backend/sbot/）
...
// 递归扫描 monorepo 的 packages/，建立 packageName -> dir 映射。
// 一旦遇到 package.json 就把当前目录视为 workspace 包，不再扫描包内部。
const workspaceRoots = [path.join(monorepoRoot, 'packages')];
...
function scanWorkspacePackages(dir) {
  ...
  if (fs.existsSync(pkgJson)) {
    ...
    if (pkg.name) workspaceMap[pkg.name] = dir;
    return;
  }
...
for (const workspaceRoot of workspaceRoots) {
  if (fs.existsSync(workspaceRoot)) scanWorkspacePackages(workspaceRoot);
}

const workspaceRootsLabel = workspaceRoots.join(', ');
...
while (queue.length > 0) {
  ...
  if (version.startsWith('file:')) {
    ...
    if (workspaceMap[name]) {
      ...
      console.warn(`warning: workspace package "${name}" not found in ${workspaceRootsLabel}`);
  ...
  for (const [name, version] of Object.entries(pkg.dependencies || {})) {
    ...
    if (typeof version === 'string' && version.startsWith('workspace:')) {
      if (workspaceMap[name]) {
        ...
        console.warn(`warning: transitive workspace package "${name}" not found in ${workspaceRootsLabel}`);
      }
    } else {
...
const distPkg = {
  ...
  releasenoteEn: releaseNote,
  releasenoteZh: releaseNoteZh,
...
// 非 release 模式：将 webui 额外复制到 packages/backend/sbot/webui 供 debug 使用
if (!isRelease && webuitSrc && fs.existsSync(webuitSrc)) {
  const debugWebuiDst = path.join(rootDir, 'webui');
