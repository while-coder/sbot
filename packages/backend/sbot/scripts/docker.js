...
if (command === 'build') {
  ...
  execSync(`docker build -t ${tag} -t ${latestTag} -f packages/backend/sbot/Dockerfile packages/backend/sbot/dist`, { stdio: 'inherit' });
  ...
  console.error('Usage: node packages/backend/sbot/scripts/docker.js <build|save|publish>');
  process.exit(1);
}
